...
contract PreMintFactory{
    ...
    uint public preMintFee;

    struct eventDetails{
        PreMintTickets Event;
        address admin;
    ...
    function addEvent(
        address owner__,
        address collection__,
        uint startTime__,
        uint endTime__
    ) external payable returns(address){
        require(msg.value == preMintFee);
        PreMintTickets instance = new PreMintTickets(
            owner__,
            reciever,
            collection__,
            startTime__,
    ...
    function getBuyerTickets(address buyer) external view returns(uint[] memory){
        ...
        for(uint i =1; i<=eventCounter; i++){
            ...
            PreMintTickets instance = PreMintTickets(eventAd);
            ERC721 ticketInstance = ERC721(instance.collectionAddress());

            list[i-1] = ticketInstance.balanceOf(buyer);
        }
        return(list);
    }

    function changeFee(uint fee_) external onlyAdmin{
        uint pastPrice = preMintFee;
        preMintFee = fee_;
